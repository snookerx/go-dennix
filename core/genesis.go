какие отличия    // Copyright 2014 The go-ethereum Authors
...
func CommitGenesisState(db ethdb.Database, triedb *trie.Database, blockhash common.Hash) error {
    ...
    if len(blob) != 0 {
        ...
        case params.DennixGenesisHash:
            genesis = DefaultDennixGenesisBlock() // Обновлено для Dennix
        }
        if genesis != nil {
            alloc = genesis.Alloc
        } else {
...
func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *trie.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
    ...
    if (stored == common.Hash{}) {
        if genesis == nil {
            ...
            log.Info("Writing custom genesis block for Dennix")
        }
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
    ...
    case ghash == params.DennixGenesisHash:
        return params.DennixChainConfig // Обновлено для Dennix
    default:
...
// DefaultDennixGenesisBlock returns the Dennix network genesis block.
func DefaultDennixGenesisBlock() *Genesis {
    return &Genesis{
        Config:     params.DennixChainConfig,
        Nonce:      0,
        Timestamp:  1735689600, // 24 февраля 2025, 21:20 MSK (UNIX-формат)
        ExtraData:  hexutil.MustDecode("0x44656e6e6978204d61696e6e6574"), // "Dennix Mainnet" в hex
        GasLimit:   200000000,
        Difficulty: big.NewInt(500000),
        Alloc: map[common.Address]GenesisAccount{
            common.HexToAddress("0x7994B72b113898AE14C78680ac0247866F04b9bE"): {Balance: new(big.Int).Mul(big.NewInt(1000000), big.NewInt(1e18))}, // 1,000,000 DEN (с 18 десятичными знаками)
        },
    }
}
