...
// IsDennix returns whether num is either equal to the Dennix fork block or greater.
func (c *ChainConfig) IsDennix(num *big.Int) bool {
    return isBlockForked(c.DennixBlock, num)
}
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, headNumber *big.Int, headTimestamp uint64) *ConfigCompatError {
    ...
    if isForkBlockIncompatible(c.DennixBlock, newcfg.DennixBlock, headNumber) {
        return newBlockCompatError("Dennix fork block", c.DennixBlock, newcfg.DennixBlock)
    }
    return nil
}
